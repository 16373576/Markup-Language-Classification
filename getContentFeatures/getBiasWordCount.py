...
biasWords = get_stop_words('gendered', 'us-common-names', 'religious', 'us-names', 'us-male-names', 'us-female-names',
                           'gendered-nouns')
...
for s in sources:
    if not os.path.isfile(
            "C:/Users/caire/Desktop/OutputData/ClassifyArticlesContentandTitle/OutputBiasCountContent/" + s + ".txt"):
            ...
            for d in p.date:
                ...
                if fileFound:  # if the source had articles on that date open all articles using articleTitles list
                    for articleTitle in articleTitles:
                        ...
                        wordCount = 0
    print(s + "'s bias words counted for each article and added to file")
